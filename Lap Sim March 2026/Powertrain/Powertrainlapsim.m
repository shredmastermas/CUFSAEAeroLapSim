...
shiftpoints = powertrainpackage{6}; % 14000; % optimal shiftpoint for most gears [RPM] (11k for lift/coast, 14 for max)
drivetrainLosses = powertrainpackage{7}; %.85;     % 15% drivetrain losses
gearSelect = 1;
gearTot = gear(gearSelect)*finalDrive*primaryReduction;     % Total gear reduction
rpm = initialV*gearTot/tyreRadius*60/(2*pi);

while gearSelect < numel(gear) && rpm > shiftpoints(gearSelect)
    gearSelect = gearSelect + 1;
    gearTot = gear(gearSelect)*finalDrive*primaryReduction;
    rpm = initialV*gearTot/tyreRadius*60/(2*pi);
end

torque = interp1(engineSpeed, engineTq, rpm, 'linear', 'extrap'); % crank torque [N-m]
