%function Total_Points = Lap_Sim_fmincon()
...
WDF = 45; % front weight distribution (%) 45
...
rg_f = 0.517; % front roll gradient (deg/g)
rg_r = 0.517; % rear roll gradient (deg/g)
pg = 0.5; % pitch gradient (deg/g)
WRF = 450; % front and rear ride rates (lbs/in)
WRR = 450; 
...
for turn = 1:1:length(radii)
    % first define your vehicle characteristics:
        ...
        beta = fzero(@(beta)diff_AY_find_1(beta, A_y, a, b, r, V, delta, full_send_y, IA_f_out, IA_f_in, IA_r_in, IA_r_out, wfin, wfout, sf_y, Cd, W, grip, wrin, wrout, T_lock, twr), 0);
        [diff_AY, M_z, AY, F_y, F_rin, F_rout, rscale, F_x, F_fout, F_fin, a_f, a_r, M_z_diff] = diff_AY_result_1(beta, A_y, a, b, r, V, delta, full_send_y, IA_f_out, IA_f_in, IA_r_in, IA_r_out, wfin, wfout, sf_y, Cd, W, grip, wrin, wrout, T_lock, twr);
        ...
        while M_z < 0 
            ...
            AY = F_y/(W/32.2);
            diff_AY = A_y-AY;
            beta = fzero(@(beta)diff_AY_find_1(beta, A_y, a, b, r, V, delta, full_send_y, IA_f_out, IA_f_in, IA_r_in, IA_r_out, wfin, wfout, sf_y, Cd, W, grip, wrin, wrout, T_lock, twr), prev_beta);
            [diff_AY, M_z, AY, F_y, F_rin, F_rout, rscale, F_x, F_fout, F_fin, a_f, a_r, M_z_diff] = diff_AY_result_1(beta, A_y, a, b, r, V, delta, full_send_y, IA_f_out, IA_f_in, IA_r_in, IA_r_out, wfin, wfout, sf_y, Cd, W, grip, wrin, wrout, T_lock, twr);
            prev_beta = beta;
        end
        prev_beta = 0;
...
%             toc
            ...
            [M_z, diff_AY] = fsolve(@(x)diff_AY_MZ_find_1(x, A_y, a, b, r, V, full_send_y, wfin, wfout, sf_y, Cd, W, grip, wrin, wrout, T_lock, twf, twr, phif, phir, IA_gainf, IA_gainr, KPIf, KPIr, deltar, IA_0f, IA_0r, casterf, casterr), [0, 0], options);
%             toc
%             disp(counter)
%             counter = counter + 1;
        end
