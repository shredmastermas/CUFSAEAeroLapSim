function [lap_time time_elapsed velocity acceleration lateral_accel gear_counter path_length weights distance] = lap_information_sprint(path_positions)
...
sections = 3000;
VMAX = top_speed;
nPathGates = min(numel(path_positions), size(path_boundaries_ax,1));
if nPathGates < 3
    error('Autocross path needs at least 3 valid gate positions.');
end
if numel(path_positions) ~= size(path_boundaries_ax,1)
    warning('Autocross path has %d positions but path_boundaries_ax has %d gates. Using the first %d positions.', ...
        numel(path_positions), size(path_boundaries_ax,1), nPathGates);
...
nSegments = length(RT);
segment = 1:nSegments;
Vmax = zeros(1,nSegments);
dist = zeros(1,nSegments);
x1 = zeros(1,nSegments);
x2 = zeros(1,nSegments);
y1 = zeros(1,nSegments);
y2 = zeros(1,nSegments);
for i = 1:nSegments
    r = max(r_min,RT(i));
    r = min(r,r_max);
    RT(i) = r;
...
nSamples = nSegments*interval;
vehicle_gear = zeros(1,nSamples);
ay_f = zeros(1,nSamples);
ax_f = zeros(1,nSamples);
dt_f = zeros(1,nSamples);
v_f = zeros(1,nSamples);
dv_f = zeros(1,nSamples);
ay_r = zeros(1,nSamples);
ax_r = zeros(1,nSamples);
dt_r = zeros(1,nSamples);
v_r = zeros(1,nSamples);
dv_r = zeros(1,nSamples);
...
dtime = zeros(1,length(VD));
acceleration = zeros(1,length(VD));
lateral_accel = zeros(1,length(VD));
time_elapsed = zeros(1,length(VD));
t_elapsed = 0;
classifier = [];
for i = 1:1:length(VD)
    if VD(i)<0
...
% set(gca,'XTick',[], 'YTick', [])
%% Gear Counter
gear_counter = zeros(1,length(velocity));
...
axis = zeros(1,length(lateral_accel));
