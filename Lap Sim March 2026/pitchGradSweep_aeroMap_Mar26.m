...
for i = 1:length(pgs)
    pg = pgs(i);
    [Total_Points,Accel_Score,Skidpad_Score,Autocross_Score,Endurance_Score,skidpad_time,AY_max,AX_max,AX_min,distance_ax,acceleration_ax,lateral_accel_ax,velocity_ax,weights_ax,vehicle_path_AX,time_elapsed_ax,brakeResults,accResults,latResults] = Lap_Sim_fminconSp26(pg);
    ...
    results = table(pgs(1:i)',totPts,accPts,skidPts,autoXPts,enduroPts,skidTime,AYmax,AXmax,AXmin,accWeight,brakeWeight,turnWeight)
end

save("latestpgResults.mat");
...
%%
% Accels
figure('Name','Accel Trace')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:height(dists)
    AYs = [Results(i).AYs].';
    plot(dists(i,1:10:end),AYs(1:10:end))
    hold on
end
xlabel('Distance [ft]')
ylabel('Lateral Accel [G]')
title('Lat Accel Trace')
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0')
grid on;
hold off
...
% % Cl Cd in Long.
% figure('Longitudinal CLCd')
% set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
% for i = 1:height(dists)
%     Vs = [Results(i).accResults.VEL].';
%     cls = -1*[Results(i).brakeResults.Cl].';
%     plot(AXs,CoPs,'-*')
%     hold on
% end
% xlabel('Distance [ft]')
...
% Aero Map
figure('Name','Aero Map')


[C,h] = contourf(X, Y, Z, 1000, EdgeAlpha=0);

xlabel('Front Ride Hight [in]')
ylabel('Rear Ride Hight [in]')
...
%set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    RHs = [vertcat(Results(i).brakeResults.RH);vertcat(Results(i).accResults.RH)];
    RHFs = RHs(:,1);
    RHRs = RHs(:,2);
    plot(RHFs,RHRs,'*')
end
...
hold off
% CoP Sensitivity
figure('Name','CoP Sensitivity (Brake)')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    Vs = [Results(i).brakeResults.VEL].';
    AXs = -1*[Results(i).brakeResults.A_Xt].';
    CoPs = [Results(i).brakeResults.Var6].';
    plot(AXs,CoPs,'-')
    hold on
end
xlabel('Longitudinal Accel [G]')
ylabel('CoP [%]')
title('CoP Sensitivity (Brake)')
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0')
grid on;
hold off

figure('Name','CoP Sensitivity (Accel)')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    Vs = [Results(i).accResults.VEL].';
    AXs = [Results(i).accResults.AXr].';
    CoPs = [Results(i).accResults.CoPs].';
    plot(AXs,CoPs,'-')
    hold on
end
xlabel('Longitudinal Accel [G]')
ylabel('CoP [%]')
title('CoP Sensitivity (Accel)')
grid on;
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0');
hold off


figure('Name','CoP Sensitivity (Lat)')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    Vs = [Results(i).latResults.speed].';
    Vs = Vs .* 0.6818;
    CoPs = [Results(i).latResults.CoPs].';
    plot(Vs,CoPs,'-')
    hold on
end
xlabel('Velocity [MPH]')
ylabel('CoP [%]')
title('CoP Sensitivity (Cornering)')
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0')
grid on;
hold off

% figure('Name','Cl/Cd (accel)')
% set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
% for i = 1:length(pgs)
%     Vs = [Results(i).accResults.VEL].';
%     Cls = [Results(i).accResults.Cls].';
%     Cds = [Results(i).accResults.Cds].';
%     ClCd = Cls./Cds;
%     plot(Vs,ClCd,'-')
%     hold on
% end
...
% ylabel('Cl/Cd')
% title('Cl vs Vel')
% legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0','Location','southeast')
% yline(0, 'k');
% grid on;
% hold off
% ylim([2,2.25])
...
% Ax vs. Velocity
figure('Name','Long. Accel Capacity [Brake]')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    Vs = [Results(i).brakeResults.VEL].';
    AXs = [Results(i).brakeResults.A_Xt].';
    CoPs = [Results(i).brakeResults.CoPs].';
    plot(Vs,AXs)
    hold on
end
xlabel('Velocity [ft/s]')
ylabel('Long. Accel [G]')
title('Long. Accel Capacity [Brake]')
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0','Location','best')
grid on;
hold off

figure('Name','Long. Accel Capacity [Accel]')
set(gca, 'ColorOrder', customColors, 'NextPlot', 'replacechildren');
for i = 1:length(pgs)
    Vs = [Results(i).accResults.VEL].';
    AXs = [Results(i).accResults.AXr].';
...
xlim([0,120])
ylabel('Long. Accel [G]')
title('Long. Accel Capacity [Accel]')
legend('pg=0.1','pg=0.2','pg=0.3','pg=0.4','pg=0.5','pg=0.6','pg=0.7','pg=0.8','pg=0.9','pg=1.0')
grid on;
hold off
