...
% Build constant scatteredInterpolants so the original aeroMapfn, lat_solve,
% lat_objective, and cAlpha_nonlcon can stay unchanged.
% The 4 surrounding points avoid needing the Excel aero map file.
RHF = [RHfi-1; RHfi+1; RHfi-1; RHfi+1];
RHR = [RHri-1; RHri-1; RHri+1; RHri+1];
cl  = CL_target  * ones(size(RHF));
cd  = CD_target  * ones(size(RHF));
cop = CoP_target * ones(size(RHF));

fnCl  = scatteredInterpolant(RHF,RHR,cl, 'linear', 'nearest');
fnCd  = scatteredInterpolant(RHF,RHR,cd, 'linear', 'nearest');
fnCoP = scatteredInterpolant(RHF,RHR,cop,'linear', 'nearest');
...
deltar = 0;
deltaf = 0;
velocity = 15:1:VMAX; % range of velocities at which sim will evaluate (ft/s)
radii = 15:5:155; % range of turn radii at which sim will evaluate (ft)
