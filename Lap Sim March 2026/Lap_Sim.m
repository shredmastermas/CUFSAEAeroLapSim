function [Total_Points,Accel_Score,Skidpad_Score,Autocross_Score,Endurance_Score,skidpad_time,AY_max,AX_max,AX_min,distance_ax,acceleration_ax,lateral_accel_ax,velocity_ax,weights_ax,vehicle_path_AX,time_elapsed_ax,brakeResults,accResults,latResults] = Lap_Sim(pg)
% clear
...
RHF = RHF(flt);
RHR = RHR(flt);
% Define scattered interpolant functions:
fnCl = scatteredInterpolant(RHF,RHR,cl);
fnCd = scatteredInterpolant(RHF,RHR,cd);
fnCoP = scatteredInterpolant(RHF,RHR,cop);
...
deltar = 0;
deltaf = 0;
velocity = 15:10:VMAX; % range of velocities at which sim will evaluate (ft/s)
...
for  i = 1:1:length(velocity) % for each velocity
    ...
    RH(i,:) = [RHf,RHr]; DFs(i,:) = [DFf,DFr]; pitches(i) = pitch/(pi/180); Vs(i) = V; CoPs(i) = CoP;
    A_Xr(i) = AX(i);
end
A_Xr(A_Xr < 0) = 0;
toc
...
%FZFs = FZFs';
%FZRs = FZRs';
accResults = table(VEL,AXr,fx,RH,DFs,pitches,CoPs)
...
x_prev = [atan(l/radii(1)), 0.02];    %initial guess for f solve
tic
for turn = 1:1:length(radii)
    ...
    while cond == 0
        ...
        fun = @(x) lat_solve(x, V, a, b, l, WDF, R, IA_staticf, IA_gainf, IA_staticr, IA_gainr, WF, WR, twf, twr, cg, W, LLTD, rg_f, rg_r, casterf, casterr, deltar, sf_y, A, grip, Cd, T_lock, KPIf, KPIr,fnCl,fnCoP,fnCd,RHfi,RHri,kRF,kRR,dxf,dxr);
        % [x, fval, exitflag] = fsolve(fun, x_prev, options);
        [x, fval, exitflag] = fsolve(fun, x_prev, options);

        % lower and upper bounds on inputs
        % [delta, belta]
        % set these to something reasonable
        lb = [deg2rad(-20), deg2rad(-12)];
        ub = [deg2rad(20), deg2rad(12)];
        ...
        if exitflag < 1 
             ...
             x_retry = [atan(l/R), 0.01];
             [x_retry_sol, fval_retry, exitflag_retry] = fsolve(fun, x_retry, options);
          if exitflag_retry >= 1 && all(abs(fval_retry) <= tol)
              x = x_retry_sol;
        ...
        if sum(x<lb) 
            % if (x(1)<lb(1))
            %     fprintf('\ndelta exceeded lower bound')
            % end
            % if (x(2)<lb(2))
            %     fprintf('\nbeta exceeded lower bound')
            % end
            cond = -1;
        elseif sum(x>ub)
            % if (x(1)>ub(1))
            %     fprintf('\ndelta exceeded upper bound')
            % end
            % if (x(2)>ub(2))
            %     fprintf('\nbeta exceeded upper bound')
            % end
            cond = -1;
        end
...
latResults = table(Rs',steering',speed',lateralg',USangle',betas',afs',ars','VariableNames',["Radius","steering","speed","latG","US Angle","Beta","af","ar"])
...
%FZFs = FZFs';
%FZRs = FZRs';
brakeResults = table(VEL,A_Xt,fx,RH,DFs,pitches,CoPs)
